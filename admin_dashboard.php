...
$query = "SELECT COUNT(*) AS pending_count FROM change_requests WHERE status = 'pending'";
$result = $conn->query($query);
$row = $result->fetch_assoc();
$pending_count = $row['pending_count'];

$query_approved_count = "SELECT COUNT(*) AS approved_count FROM change_requests WHERE status IN ('approved', 'rejected')";
$result_approved_count = $conn->query($query_approved_count);
$row_approved = $result_approved_count->fetch_assoc();
...
.request-box {
    ...
    padding: 30px;  /* เพิ่มการ padding ให้มากขึ้นเพื่อให้กล่องสูงขึ้น */
    border-radius: 0;  /* ลบการโค้งมุมออกให้เหลี่ยมตรง */
    box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);  /* เพิ่มเงาที่คมชัดขึ้น */
    margin-bottom: 20px;
    text-align: center;
    height: 250px; /* เพิ่มความสูงของกล่อง */
    display: flex;
    flex-direction: column;
    justify-content: center; /* จัดเนื้อหากลางในแนวตั้ง */
    align-items: center; /* จัดเนื้อหากลางในแนวนอน */
}

/* สไตล์หัวข้อ */
h3 {
    font-size: 22px; /* เพิ่มขนาดตัวอักษร */
    color: #333;
    margin-bottom: 15px;
}
...
/* ให้กล่องในแต่ละคอลัมน์แสดงเรียงกันเป็นสองคอลัมน์ */
...
<body class="sub_page">
  ...
  <div class="container">
    <div class="row">
        <!-- กล่องคำร้องที่รอดำเนินการ -->
        <div class="col-md-6">
            <div class="request-box pending">
                <h3>Pending Petitions</h3>
                <p>you have <strong><?php echo $pending_count; ?></strong> New pending requests</p>
                <a href="manage_requests.php" class="btn btn-primary">Manage requests</a>
            </div>
        </div>

        <!-- กล่องคำร้องที่ยืนยันแล้ว -->
        <div class="col-md-6">
            <div class="request-box approved">
                <h3>Confirmed Request</h3>
                <p>you have <strong><?php echo $approved_count; ?></strong> Confirmed request</p>
                <a href="approved_requests.php" class="btn btn-success">View confirmed requests</a>
            </div>
        </div>
    </div>
</div>
